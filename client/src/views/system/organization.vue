<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'编辑部门':'新增部门'" >
      <el-form :model="org" label-width="80px" label-position="right" :rules="rule1" ref="Form">
  ...
  methods: {
    ...
    getList() {
      ...
      getOrgAll().then(response => {
        ...
        let data = genTree(response.data);
        this.tableData = data;
        this.pidList = data
    ...
    handleFilter() {
      let newData = this.orgList.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
